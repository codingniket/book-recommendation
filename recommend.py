...
class Recommender:
    ...
    def _load_books_and_vectorstore(self):
        """
        Lazy loads books and vector store only on first use.
        """
        if self.books is None or self.vectorstore is None:
            with self.Session() as session:
                books_list = session.query(Book).all()
                data = [{
                    'isbn13': book.isbn13,
                    ...
                    'thumbnail': book.thumbnail,
                    'description': book.description,
                    'tagged_description': book.tagged_description  # keep if needed
                } for book in books_list]

                self.books = pd.DataFrame(data)
                self.books['isbn13'] = self.books['isbn13'].astype(str)

                print(f"[Recommender] Loaded {len(self.books)} books from DB.")

                if self.books.empty:
                    print("[Recommender] Warning: No books loaded from DB. Recommendations may fail.")
                ...
                # Persist directory helps keep the vectorstore cached between runs
                self.vectorstore = Chroma.from_documents(documents, embeddings, persist_directory="./chroma_db")
                print(f"[Recommender] Loaded {len(documents)} documents into vector store.")

    def get_recommendations(self, query, top_k=5, raw=False):
        self._load_books_and_vectorstore()

        if self.books.empty:
            return []

        print(f"[Recommender] User query: '{query}'")

        keywords = set(query.lower().split())

        results = self.vectorstore.similarity_search_with_score(query, k=30)
        print(f"[Recommender] Found {len(results)} raw results from vector store.")
        ...
        for res, score in results:
            ...
            # Boost score based on keyword matches in title, categories, description
            boost = 0
            for kw in keywords:
                if pd.notna(book['title']) and kw in book['title'].lower():
                    boost += 1.5
            ...
            # Adjust score: lower is better in Chroma
    ...
    def rebuild_vectorstore(self):
        """
        Refreshes books and vectorstore, e.g. after book additions.
        """
        try:
            with self.Session() as session:
                books_list = session.query(Book).all()
                ...
                print(f"[Recommender] Rebuilding vector store with {len(self.books)} books.")
                ...
                print("[Recommender] Vector store rebuilt.")

        except Exception as e:
            print(f"[Recommender] Rebuild failed: {str(e)}")
            raise
